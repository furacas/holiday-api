...
public interface CalendarRepository extends JpaRepository<Calendar, Long> {
     ...
     * @return
     */
    @Query(value = "select * from calendar where date >= :date and name like concat('%',:name,'%') order by date asc limit 1",nativeQuery = true)
     ...
     * @return
     */
    @Query(value = "select * from calendar where date >= :date and status = 'HOLIDAY' order by date asc limit 1",nativeQuery = true)
