...
public class CalendarService implements ApplicationRunner {
  ...
  private static final String BAIDU_API =
      "https://sp0.baidu.com/8aQDcjqpAAV3otqbppnN2DJv/api.php?query={0}&co=&resource_id=39043&t=1617089428269&format=json&tn=wisetpl";
  ...
  private void updateCalendar(Integer year, Integer month) {
    String encoderParam =
        URLEncoder.encode(
            MessageFormat.format(QUERY_DATE, year.toString(), month.toString()),
    ...
    for (BaiduCalendarResponse.Almanac at : almanac) {
      ...
      calendarRepository
          .findByDayAndMonthAndYear(calendar.getDay(), calendar.getMonth(), calendar.getYear())
          .ifPresentOrElse(
              (exist) -> {
