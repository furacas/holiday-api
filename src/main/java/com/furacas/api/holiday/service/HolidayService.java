...
public class HolidayService {
  ...
  public HolidayInfo getHolidayInfo(String date) {
    ...
    return calendarRepository.findByDate(queryDate).map(this::convertToInfo).orElse(null);
  }

  public HolidayInfo getNextHolidayInfo(String name) {
    if (Objects.isNull(name)) {
      return calendarRepository.findNextHoliday(new Date()).map(this::convertToInfo).orElse(null);
    }
    return calendarRepository
        .findNextHoliday(new Date(), name)
